...
class FractionSumFinder:
    ...
    def find_best_combination(self):
        ...
        while len(max_mins) < 2:
            ...
            min_list_combo = self.random_extreme(valid_remove_combinations, min)
            max_valid_combo = self.random_extreme(valid_add_combinations, max)
            # min_list_combo = min(valid_remove_combinations, key=len)
            # max_valid_combo = max(valid_add_combinations, key=len)
        ...
        # same_scores = []
        # for i in range(len(mm_score)):
        #     if mm_score[i] == max(mm_score):
        #         same_scores.append(max_mins[i])
    ...
    def find_fractions(self):
        ...
        for set_item in self.number_set.intersection(
            ...
            # max_next_list = list(filter(lambda x: x == max(next_lists), next_lists))
            # max_next_list = max_next_list[randint(0, len(max_next_list)-1)]
