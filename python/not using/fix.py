...
class FractionSumFinder:
    ...
    def find_best_combination(self):
        ...
        while len(mm_score) < 5:
            ...
            def is_subset(c):
                return set(c).issubset(self.number_set)

            def is_disjoint(c):
                return set(c).isdisjoint(self.number_set)
        ...
        return set(self.number_set).difference(best_min).union(best_max)
    ...
    def main(self):
        ...
        # def not_in_list(c):
        #     return all(number not in self.number_set for number in c)
        ...
        while True:
            ...
            for index in list(self.number_set):
                self.index = index
                combinations = self.cached_combinations.get(index, set())
                # valid_combinations = set(filter(lambda c: not_in_list(c), combinations))
                valid_combinations = set(filter(lambda c: set(c).isdisjoint(self.number_set), combinations))
                ...
                max_combination = set(list(valid_combinations)[max_next_sum_idx])
                self.number_set.remove(index)
                self.number_set = set(self.number_set) | set(max_combination)
