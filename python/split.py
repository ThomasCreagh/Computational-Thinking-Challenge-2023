...
class FractionSumFinder:
    ...
    def removing_numbers_check(self):
        for get_rid_of_number in self.number_set:
            ...
            for children in get_rid_of_parents:
                ...
                valid_add_combinations = list(filter(
                    lambda c: get_rid_of_number not in c,
                    valid_add_combinations))

                valid_remove_combinations = list(filter(
                    lambda c: get_rid_of_number in c,
                    valid_remove_combinations))
                ...
                if (len(max(valid_add_combinations, key=len))
                    ...
                    # self.can_remove.append((
                    #     get_rid_of_number,
                    #     [max(valid_add_combinations, key=len),
                    #      min(valid_remove_combinations, key=len)]))
                    can_get_rid_of += 1

            if can_get_rid_of != 0:
                # self.can_remove.append(get_rid_of_number)
                can_get_rid_of = 0
        # self.can_remove = {235: [[[300, 483, 575], [235, 420, 1974]]], 242: [[[348, 374, 493], [242, 484, 726]]], 330: [[[348, 374, 493], [330, 420, 462]], [[300, 483, 575], [330, 385, 660]]], 350: [[[300, 483, 575], [350, 420, 525]], [[375, 750], [350, 875]]], 360: [[[300, 483, 575], [360, 420, 504]]], 363: [[[348, 374, 493], [242, 363, 1452]]], 372: [[[300, 483, 575], [372, 434, 465]]], 385: [[[300, 483, 575], [330, 385, 660]]], 406: [[[300, 483, 575], [406, 420, 435]]], 420: [[[348, 374, 493], [330, 420, 462]], [[300, 483, 575], [350, 420, 525]]], 434: [[[300, 483, 575], [372, 434, 465]]], 435: [
        #     [[300, 483, 575], [406, 420, 435]]], 462: [[[348, 374, 493], [330, 420, 462]], [[300, 483, 575], [385, 420, 462]]], 465: [[[300, 483, 575], [372, 434, 465]]], 470: [[[300, 483, 575], [235, 470, 1316]]], 484: [[[348, 374, 493], [242, 484, 726]]], 504: [[[300, 483, 575], [360, 420, 504]]], 525: [[[300, 483, 575], [350, 420, 525]]], 660: [[[300, 483, 575], [330, 385, 660]]], 726: [[[348, 374, 493], [242, 484, 726]]], 875: [[[375, 750], [350, 875]]], 1316: [[[300, 483, 575], [235, 470, 1316]]], 1452: [[[348, 374, 493], [242, 363, 1452]]], 1974: [[[300, 483, 575], [235, 420, 1974]]]}
    ...
    def replace_smartly(self):
        ...
        for i in replacables:
            lst = []
            for j in i[1]:
                lst += self.can_remove[j]
            connection_list.append(lst)

        print(connection_list)
        connection_list_2 = []
        for need_replacing in connection_list:
            valid = True
            for pair in need_replacing:
                for combination in pair:
                    other_pair_index = 1
                    while other_pair_index < len(need_replacing):
                        # for other_pair_index in range(1, len(need_replacing)):
                        other_pair = need_replacing[other_pair_index]
                        for other_comination in other_pair:
                            if (len(set(combination).intersection(
                                    set(other_comination)))):
                                valid = False

                                # removal_pair = connection_list.index(
                                #     need_replacing)

                                # connection_list[removal_pair].remove(pair)
                                # other_pair_index += 1

                        other_pair_index += 1
            if valid:
                connection_list_2.append(need_replacing)

        new_connection_list = []
        for items in connection_list_2:
            if len(items) < 2:
                continue
            new_connection_list.append(items)

        print(new_connection_list)
        if len(new_connection_list):
            print("It will replace stuff")
        else:
            print("No replacing ")

        for items in new_connection_list:

            # checking_for_divergence = list(filter(
            #     lambda c: c in self.number_set, items[1]))

            # main_continue = False
            # for i in items[1]:
            #     for j in self.can_remove[i]:
            #         valid = True
            #         for k in j:
            #             if items[0] in k:
            #                 valid = False
            ...
            #             if l == i:
            #                 continue
            #             for m in self.can_remove[l]:
            #                 # for n in m:
            #                 if len(set(j[0]).intersection(m[0])):
            #                     break_out = True
            # if break_out:
            #     continue

            # self.can_remove[i].remove(j)
            if len(new_connection_list):
                print("It will replace stuff")
            else:
                print("No replacing ")
            ...
            #         print(self.can_remove[number])
            #         # if not len(self.can_remove[number]):
            #         #     break
            #         add_to_set = self.can_remove[number][0][0]

            #         remove_from_set = self.can_remove[number][0][1]

            #         self.number_set = self.number_set.difference(
            #             set(remove_from_set)).union(
            #             set(add_to_set))

            #         print("remove:", remove_from_set)
            #         print("adding:", add_to_set)
            #         print()
    ...
    def find_best_combination(self):
        ...
        while len(max_mins) < 1:
            ...
            max_valid_combo = self.random_extreme(valid_add_combinations, max)
            max_mins.append((len(max_valid_combo) - len(min_list_combo),
                             min_list_combo, max_valid_combo))

        mm_score = [i[0] for i in max_mins]
        if max(mm_score) >= 0:
            max_rand = list(filter(lambda x: x[0] == max(mm_score), max_mins))
            _, best_min, best_max = max_rand[randint(0, len(max_rand)-1)]
            self.number_set = self.number_set.difference(
